...
def extract_contract_os_num(text):
    contract_match = re.search(r'Contrato: ([\w\-]+ \d{4}\/\d{2})', text)
...
def extract_discipline(text):
    # Procurar a disciplina no formato: "disciplina de [DISCIPLINA]:"
    discipline_match = re.search(r"disciplina de ([A-Za-z]+):", text)
    if discipline_match:
        return discipline_match.group(1).upper()  # Retornar a disciplina em maiúsculo
    return None
...
def save_ncs_to_template_excel(ncs, template_path, excel_path, contract, os_value, date, num_value, inspection_text, discipline):
    ...
    for i, nc in enumerate(ncs, start=1):
        ...
        new_sheet.cell(row=12, column=1, value=f"NC {i}")
