...
class NCExtractorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Extrair NCs Consolidadas")
        ...
        # Botão para extrair NCs
        tk.Button(root, text="Extrair NCs Consolidadas", command=self.run_extraction).pack(pady=20)
    ...
    def run_extraction(self):
        ...
        output_file = os.path.join(output_folder, "NCs_Consolidadas.xlsx")
        
        try:
            consolidate_ncs_data(input_files, model_file, output_file)
...
def consolidate_ncs_data(input_files, model_file, output_file):
    ...
    for input_file in input_files:
        ...
        for sheet_name in wb_input.sheetnames:
            if sheet_name.startswith("NC "):
                ...
                # Adiciona os dados nas colunas A, D e L da linha atual
