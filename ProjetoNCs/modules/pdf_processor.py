...
class PDFProcessor:
        ...
        # Função para conectar ao banco de dados SQL Server com debugclass PDFProcessor:
    def conectar_banco(self):
        try:
            conn_str = 'DRIVER={ODBC Driver 18 for SQL Server};SERVER=SAP3SQLW2K01.eec.local;DATABASE=DPE;UID=DPEUSR;PWD=Js&3h8YFR;Encrypt=no'
            conn = pyodbc.connect(conn_str)
            return conn
        except pyodbc.Error as e:
            messagebox.showerror("Erro de Conexão", f"Erro ao tentar conectar ao banco de dados: {e}")
            return None

    def inserir_dados_nc(self, ncs_para_inserir):
        try:
            conn = self.conectar_banco()
            if conn:
                cursor = conn.cursor()
                # Usando executemany para inserir em bloco
                cursor.executemany(
                    "INSERT INTO nc (descricao_nc, disciplina_nc, numero_nc, ri_id) VALUES (?, ?, ?, ?)",
                    ncs_para_inserir
                )
                conn.commit()
                cursor.close()
    ...
    def inserir_dados_ri(self, ri_inserir):
        try:
            conn = self.conectar_banco()
            if conn:
                cursor = conn.cursor()
                ...
                for data in ri_inserir:
                    cursor.execute(
                        "INSERT INTO ri (num_relatorio, data_relatorio, nome_arquivo, revisao_relatorio) "
                ...
                messagebox.showinfo("Sucesso", "Dados de RI inseridos com sucesso!")
                return inserted_ids
        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao inserir dados de RI: {e}")
            return []


    def enviar(self, _event=None):
        try:
            pdf_paths = self.pdf_paths_var.get().split("; ")
            if not pdf_paths or pdf_paths == ['']:
                messagebox.showwarning("Aviso", "Por favor, selecione os PDFs antes de enviar os dados.")
                return
            ...
            for pdf_path in pdf_paths:
                try:
                    ...
                    if ncs:
                        for nc in ncs:
                            nc_completa = nc
                            split_num_nc = nc.split(".")
                            numero_nc = split_num_nc[0]
                            descricao_nc = split_num_nc[1]

                            ncs_para_inserir.append((nc_completa, discipline, numero_nc))

                        ri_inserir.append((num_value, date, output_excel_filename, rev_value))

                except Exception as e:
                    messagebox.showerror("Erro", f"Erro ao processar o PDF '{os.path.basename(pdf_path)}': {str(e)}")
                    return

            # Após inserir dados em `ri`
            ri_ids = self.inserir_dados_ri(ri_inserir)

            if ri_ids:
                # Crie uma nova lista para os NCS com RI associado
                ncs_com_ri = []
                for ri_id in ri_ids:
                    for nc_data in ncs_para_inserir:
                        ncs_com_ri.append((*nc_data, ri_id))  # Adiciona o ID do RI ao NC

                # Insere os NCs associados ao RI
                self.inserir_dados_nc(ncs_com_ri)


            messagebox.showinfo("Sucesso", "Todos os PDFs foram processados com sucesso!")

        except Exception as e:
            messagebox.showerror("Erro", f"Erro ao enviar os dados: {str(e)}")
    ...
    def __init__(self, root):
        ...
        tk.Button(root, text="Processar PDFs", command=self.processar_e_registrar, bg="green", fg="white").pack(pady=20)
        

    
    
    def processar_e_registrar(self):
        self.process_pdfs()  # Primeiro, chama a função de processar os PDFs
        self.enviar()        # Depois, chama a função de registrar no banco
